...
local abilities = {
	...
	["SteadyShot"] = function()
		...
		if ValidUsable(spells.SteadyShot.id, "target") and _playerInRange and not ni.player.ismoving() then
			if
				IsSpellKnown(spells.ExplosiveShot.id) and ni.spell.cd(spells.ExplosiveShot.id) > 1.8 and
					FacingLosCast(spells.SteadyShot.name, "target")
			 then
				return true
			end
			if not IsSpellKnown(spells.ExplosiveShot.id) and FacingLosCast(spells.SteadyShot.name, "target") then
				return true
			end
		end
	end,
...
ni.bootstrap.profile("君莫笑 PvE 生存猎人", queue, abilities, OnLoad, OnUnload)
...
[15:44] DarhangeR:
{ type = "page", number = 2, text = "|cffEE4000Настройка ротации|r"},
]]
