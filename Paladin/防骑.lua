...
local fushi, mingling, zhengyi, zhihui, guangming = "|cffFFFF33腐蚀圣印", "|cffBF211E命令圣印", "|cff24E0FB正义圣印","|cff7189ff智慧圣印","|cffE6FDFF光明圣印"
...
SealofInsight = {id = 20165, name = GetSpellInfo(20165)},--光明圣印
...
AvengersShield = {id = 31935, name = GetSpellInfo(31935)},--复仇者之盾
...
HammerofJustice = {id = 853, name = GetSpellInfo(853)},--制裁之锤
...
SealofCommand = {id = 20375, name = GetSpellInfo(20375)},--命令圣印
...
local abilities = {
	["Pause"] = function()
		if IsMounted()
			...
			or ni.unit.buff("player", item.drink.a)
			or ni.unit.buff("player", item.drink.b)
	...
	["UseHpPotions"] = function()
        if enables["UseHpPotions"] then
            for v,i in ipairs(item.hppotions) do
                if (i ~= 0 and i ~= nil) and ni.player.hasitem(i) and
                ni.player.itemcd(i) == 0 and
                ni.player.hp() < values["UseHpPotions"] then
                    ni.player.useitem(i)
                return true
                end
            end
	...
	["agro"] = function()
		...
		and incombat
		and ni.spell.cd(spells.RighteousDefense.id)
		and not UnitIsUnit("targettarget","player")
		and ni.spell.available(spells.HandofReckoning.id)
		and ni.spell.valid("target",spells.HandofReckoning.id,true,true) then
		...
		if ni.unit.exists("target")
		and incombat
		and GetTime() - lastTime > 0.5
		and not UnitIsUnit("targettarget","player")
		and not ni.spell.available(spells.HandofReckoning.id)
		and ni.spell.cd(spells.HandofReckoning.id)
	...
	["HandofSalvation"] = function()
		if enables["HandofSalvation"] and incombat then
			if ni.player.hp() < values["HandofSalvation"]
			and ni.spell.available(spells.HandofSalvation.id) then
	...
	["AvengingWrath"] = function()
		if incombat and ni.unit.isboss("target")
		and ni.spell.available(spells.HolyVengeance.id)
		and ni.unit.debuffstacks("target", spells.HolyVengeance.name, "player") >= 4 then
	...
	["HandofSacrifice"] = function()
        if enables["HandofSacrifice"] and incombat then
            for i = 1, #ni.members do
                if not ni.members[i].istank
				and ni.player.hp() > 60
                and ni.members[i].hp < values["HandofSacrifice"]
                and ni.spell.available(spells.HandofSacrifice.id)
                and ni.spell.valid(ni.members[i].unit, spells.HandofSacrifice.id, false, true, true) then
	...
	["LayonHands"] = function()
        if enables["LayonHands"] and incombat then
			for i = 1, #ni.members do
                if ni.members[i].hp < values["LayonHands"]
                and not ni.unit.debuff(ni.members[i].unit, 25771)
                and ni.spell.available(spells.LayonHands.id)
                and ni.spell.valid(ni.members[i].unit, spells.LayonHands.id, false, true, true) then
	...
	["HandofProtection"] = function()
		if enables["HandofProtection"] and incombat then
			for i = 1, #ni.members do
                if ni.members[i].hp < values["HandofProtection"]
				and not ni.members[i].istank
				and not ni.unit.debuff(ni.members[i].unit, 25771)
				and ni.spell.available(spells.HandofProtection.id)
				and ni.spell.valid(ni.members[i].unit,spells.HandofProtection.id,false,true,true) then
	...
	["Seal"] = function()
		...
		if ni.player.power("mana") < 20
			and ni.spell.available(spells.SealofWisdom.id)
			and not ni.player.buff(spells.SealofWisdom.id) then
				ni.spell.cast(spells.SealofWisdom.name)
				return true
		end
		--腐蚀
		if s == 1
			and ni.player.power("mana") > 90
		...
		and ni.spell.available(spells.SealofCommand.id)
		and not ni.player.buff(spells.SealofCommand.id) then
	...
	["SacredShield"] = function()
		if incombat
			and ni.unit.buffremaining("player", spells.SacredShield.id) < 2
			and ni.spell.available(spells.SacredShield.id)
		then
	...
	["Avengers Shield"] = function()
		if ni.player.power("mana") > 40
		and ValidUsable(spells.AvengersShield.id, "target")
	...
	["ShieldoftheRighteous"] = function()
		...
		and FacingLosCast(spells.ShieldoftheRighteous.name, "target") then
			return true;
		end
	end,
	...
	["Judgement"] = function()
		...
		if s == 1
		and ValidUsable(spells.Judgement.id, "target")
		and FacingLosCast(spells.Judgement.name, "target") then
			return true;
		end
		if s == 2
		and ValidUsable(spells.wisdomJudgement.id, "target")
		and FacingLosCast(spells.wisdomJudgement.name, "target") then
			return true;
		end
	end,
	["Consecration"] = function()
		if enables["Consecration"]
		and incombat
		and ni.player.power("mana") > 30
		and ni.spell.available(spells.Consecration.id)
		and ActiveEnemies() >= values["Consecration"] then
	...
	["HammerofJustice"] = function()
		...
		and ni.spell.available(spells.HammerofJustice.id) then
            ...
            for i = 1, #enemies do
                ...
                if ni.spell.shouldinterrupt(InterruptTargets,values["HammerofJustice"])
